...
- **IV-12 Surface lifecycle (§14.3).** Flight-skew items dismiss themselves when their condition resolves. Preservation items (§4.2, §5.4) resolve only by acknowledgment. No item outlives its condition.
...
- **LG-11 [E]** Rebuild, resource absent: on `404`, the rebuilt record is a remote-observed tombstone. The note becomes orphaned. The accumulated dirty state goes inert. Run this test against a setup near-identical to LG-10's setup. Use this setup to demonstrate that the branches cannot fall through each other (row 16b).
...
- **LP-3 [E]** Token rejection: if the CTag is unchanged, no query occurs, and this is the cheap exit. If the CTag changed, a horizon `calendar-query` occurs, and the diff is scoped to in-horizon records. Out-of-horizon records stay untouched: no reads and no writes occur against them (§5.1).
...
- **CD-9 [E]** External edits made while Obsidian was closed appear as files changed before startup. These edits create no dirty entry. They take the surfaced path. An auto-push never happens (§5.3).
...
- **IN-8 [E]** A remote deletion arrives for an identity with a linked note, under the flag default. Davenport writes a remote-observed tombstone. Davenport puts a banner on the note. The file survives. The materialized sections stay (§5.4, §5.6, §7.5).
...
- **PU-2 [E]** A creation targets `{uid}.ics` and carries `If-None-Match: *`. A `412` on creation makes Davenport fetch and converge. Davenport strips `state`. Davenport reports no error (row 4, §5.5).
...
- **TK-2 [E]** A block is a VEVENT with a `task:` wikilink. The default is `OPAQUE`. When the user completes a block, the plugin offers task completion. The plugin makes an offer only. If the user declines the offer, the plugin writes nothing. (§10.1, §13)
- **TK-3 [E]** A dangling `task:` link renders plain. The plugin suppresses the completion offer. The plugin shows no error. (row 23)
...
- **TK-7 [C]** This test covers the crash windows around the mandated order. If the crash occurs before the tombstone, the plugin writes nothing and the user can run the command again. If the crash occurs between the tombstone and the rewrite, an orphaned note stays with the "converted" annotation. If the successor stays unresolvable past the flight grace, the plugin shows the incomplete-conversion surface. That surface offers complete (a re-derived rewrite) and revert. Revert follows the §5.6 keep; after the `DELETE`, revert is a fresh push, and the plugin states it as such. If the crash occurs after both steps, normal decomposed processing follows. (row 13b, §10.4)
...
- **TK-13 [E]** Conversion requires a linked note. The plugin refuses a venue-routed record and states the reason. (§10.4)
- **TK-14 [E]** The command's rewrite never trips the §6.1 hand-edit guard (self-write exclusion). (§10.4, §5.3)
- **TK-15 [E]** This test covers a move to a calendar. The plugin runs pre-validation at the target. The plugin writes the tombstone first, with the "moved to" annotation. The note rewrite mints a fresh `uid`, and the test asserts that this `uid` is distinct from the old identity. The note rewrite changes `calendar:` only, and every other field stays byte-untouched. The venue pointer carries forward. The server work completes as standard tombstone processing plus a push, and the mock's request log shows delete-plus-create, never `MOVE`. (§10.4)
...
- **RC-1 [D]** The plugin computes the instances from `RRULE` for display. The materialization map keys by instance date. (§11)
- **RC-2 [D]** The closure rule applies to edits of the series `start`, `rrule`, or `timezone`, and it includes the timed↔all-day key swap. While overrides or exclusions exist, the plugin refuses these edits and states the reason. Safe fields patch. (§11)
- **RC-3 [D+E]** The plugin preserves overrides and `EXDATE`s across a round trip from day one, under modeled-field pushes. (IV-4) (§11, §5.5)
- **RC-4 [D]** The plugin renders the preserved overrides. Views apply the existing overrides and exclusions read-only. A moved instance displays as moved. (§11)
- **RC-5 [E]** The plugin honors the complete-and-respawn convention for an inbound recurring VTODO. (§11)
- **RC-6 [D]** The timezone matrix follows Part 6.5. The plugin computes DST-straddling instances and all-day instances in the event's zone. The plugin never computes them in the device's zone. (§11)

### 5.21 RSVP [RS] — §12

- **RS-1 [E]** Pending detection uses the own `ATTENDEE` with `PARTSTAT=NEEDS-ACTION`, and it matches by address. The plugin shows a banner and a Needs Response listing. (§12, §14.3)
- **RS-2 [E]** If the user sets `rsvp:` and no own-`ATTENDEE` match exists, validation fails. The failure states the alias hint. The plugin writes nothing. (row 21, §6.2)
- **RS-3 [E]** A response is a confirm-gated server action. The buttons and the hand-edited `rsvp:` key are equivalent signals. The plugin validates the enum. (§12)
...
- **PM-3 [D]** This test covers the `transp`, `class`, `status`, and `location` mappings. The plugin preserves structured-location `X-` properties. (IV-4) (§13)
...
- **UI-3 [U]** This test covers the sync activity. The plugin derives the pending count. No queue artifact exists anywhere on disk. The log records refusals, skips, and conflicts, and not only successes. The filters work. (§14.4)
...
- **UI-7 [U]** Drag and resize in the calendar view route through the standard edit path: debounce, validation, and the attendee gate. The plugin disables drag and resize on remote-owned events. The plugin shows the reason on the attempt. The handle does not disappear silently. (§14.2)
- **UI-8 [U]** The plugin renders drafts dashed, `tentative` faded, `cancelled` struck, and blocks patterned. The RSVP, conflict, and validation badges are present. (§14.2)
...
- **UI-11 [U]** This test covers the modals, the full §14.6 set. Quick-add previews the parse results live, and Esc abandons quick-add with zero writes. The user cannot suppress the attendee confirmation. The retraction options state their consequences. The conflict table resolves per field or wholesale. The adoption picker pins the fuzzy-match suggestions on top. The venue picker searches the notes. The dangling-pointer modal offers retract, unlink, and restore. (§14.6, §15.4)
...
- **UI-16 [U]** This test covers the calendar-view interactions beyond drag and resize. When the user clicks an event, the plugin opens its materialized note or offers materialization. When the user clicks a draft, the plugin opens its note. When the user drags empty space, the plugin opens quick-add, pre-filled with the selected time and with the view's calendar context. The context menu carries the §14.2-enumerated actions. Each action is visible only where it applies. (§14.2)
...
- **SI-1 [E]** The stages 2–3 interim rule is this: the plugin defers and flags inbound changes to notes in the local dirty set. The plugin never applies these changes over dirty local state. This test exists only in stage-2/3 builds. The Part 5.10 suite replaces this test when three-way comparison lands in stage 4. (§18)
