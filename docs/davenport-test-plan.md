...
- **LG-14 [U]** The onboarding excluded-files button adds the records folder. The button adds the folder that the setting names at the moment of the press. A records-folder path change leaves Obsidian's excluded-files setting alone, so the entry for the old folder stays until the user removes it (§15.2). The documentation of the Bases/Dataview trade-off follows A18's recorded fact (§3.2).
...
- **CD-1 [E]** Watcher scope: the watcher observes only notes that carry `uid` or `state`. The watcher does not watch plain notes, and this includes acknowledged notes with event fields intact (row 1, B axes). The watcher does not watch a record file inside the records folder. That file carries `uid` and no `state`, and Davenport does not treat it as a note (§5.3). The exclusion names the folder and not the shape, so a user file inside the records folder is not a note either, whatever keys it carries. Every note-side rule stays inert for that file, and this test asserts the inertness by its consequences: the file produces no dirty entry, no pending-set member, no push, no validation failure, no item in Needs Attention, and no write to the file, which includes the duplicate-identity recovery of §3.4. A path change moves the folder exclusion with the setting (§15.2). A user file left behind in the old folder is a note again from that moment, and the watcher observes it when it carries `uid` or `state`. A record left behind is not: a file that parses as a record stays outside note handling in every folder, no note-side rule writes to it, and Davenport surfaces it as a record in the wrong folder. Construct that case from a failed move-back and from a record that vault sync delivers to a folder the setting does not name (§5.3, §3.4).
...
- **CF-1 [E]** This test covers the tier discipline. Device-local view-state changes cause zero `data.json` writes. Every synced-settings change writes the revision marker. A records-folder path change is one such change, and this test covers the whole of that change. A change of the setting refuses a target folder that holds a user file, at any depth, and the scan sees vault files only: a file of the operating system and a marker file of a sync tool refuse nothing. A file that parses as a record is not a user file, whether or not that record quarantines, so a conflict copy in the target refuses nothing either. A target folder that contains the settings-revision marker is refused. The refusal gates a change and gates nothing else: a device that starts against a records folder it did not choose runs no check, writes records normally, and a conflict copy that arrives after the change goes to quarantine rather than to a refusal. The change also refuses nesting with a configured path in both directions, and the equal-path case is one of them. The template-folder setting, the person-index scope, and a routing rule each refuse a value that nests with the records folder, so both directions have a gate. A daily-note folder that nests raises a surface instead, because Davenport owns no gate on that setting. A folder setting that holds no value takes no part in the nesting check, and the vault root is refused even when every other folder setting is empty. An accepted change moves every ledger file and every quarantined file to the new folder byte for byte, writes no record while the move runs, and writes the new value of the setting only after the last file arrives. The ledger index and the quarantine scanner then read the new folder. A file that neither the ledger nor the quarantine holds stays in the old folder. A failed move leaves the setting at its old value, reports the file that failed, and moves back every file that it already moved. A kill during the move (Part 3.6) leaves the setting at its old value and leaves records in the new folder; those files stay outside note handling, and Davenport surfaces each one it meets. (§15.1, §15.2, §3.2, §5.3)
- **CF-2 [E]** This test covers the tripwire. A marker ahead of the local revision surfaces the stale-settings warning. The warning names the divergent sections and calls out ownership-mode divergence. A damaged marker costs at most one wrong warning, and the next settings write corrects that warning. The marker never gates behavior and never appears in quarantine. The marker lives outside the records folder by design, and the refusal of §15.2 keeps that fact true: a target folder that contains the marker is refused, because the marker does not parse as a record. A device that does not receive a new records-folder path keeps the old path, and the stale-settings warning is that device's surface for the change. (§15.1, §15.2, §3.2)
...
- **Stage 4 (conflicts):** IN complete, and Part 6.4 fully exercised; SI-1 retired; the preservation items (IN-3, RG-6, DL-2, and the preservation half of UI-2: stage 1 builds the class of items that only an acknowledgment dismisses, and stage 4 gives that class its members), and the conflict UI (the UI-11 table, plus the stage-4 variant of PU-7, which replaces the stage-3 behavior); IV-10/IV-12 at full strength.
