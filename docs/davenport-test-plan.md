...
- **CF-1 [E]** This test covers the tier discipline. Device-local view-state changes cause zero `data.json` writes. Every synced-settings change writes the revision marker. A records-folder path change is one such change, and this test covers the whole of that change. The setting refuses a target folder that holds a file which is not a record of the plugin, at any depth. A record is a file whose name equals the digest of the identity it holds and whose self-checksum verifies. The setting refuses nesting with another configured folder in both directions, and the equal-path case is one of them. A folder setting that holds no value takes no part in the nesting check. An accepted change moves every ledger file and every quarantined file to the new folder byte for byte, and it writes the new value of the setting only after the last file arrives. The ledger index and the quarantine scanner then read the new folder. A file that neither the ledger nor the quarantine holds stays in the old folder. A failed move leaves the setting at its old value, reports the file that failed, and moves back every file that it already moved. (§15.1, §15.2, §3.2)
- **CF-2 [E]** This test covers the tripwire. A marker ahead of the local revision surfaces the stale-settings warning. The warning names the divergent sections and calls out ownership-mode divergence. A damaged marker costs at most one wrong warning, and the next settings write corrects that warning. The marker never gates behavior and never appears in quarantine. The marker lives outside the records folder by design, and the refusal of §15.2 holds that fact: a target folder that holds the marker is refused, because the marker is not a record. A device that does not receive a new records-folder path keeps the old path, and the stale-settings warning is that device's surface for the change. (§15.1, §15.2, §3.2)
