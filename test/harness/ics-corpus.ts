...
type IcsFixtureEntry = Omit<IcsFixture, 'path' | 'text'>;

const INDEX: readonly IcsFixtureEntry[] = [
	{
		name: 'x-props-vendor-names',
	...
	{
		name: 'structured-location-apple',
	...
	{
		name: 'fold-splits-multibyte-run',
		categories: ['folding-and-escaping'],
		summary:
...
const FIXTURE_DIR = join(import.meta.dirname, 'fixtures', 'ics');
...
export function icsFixture(name: string): IcsFixture {
	const found = icsCorpus().find((fixture) => fixture.name === name);
	if (found === undefined) {
...
function readFixture(entry: IcsFixtureEntry): IcsFixture {
	const path = join(FIXTURE_DIR, `${entry.name}${FIXTURE_EXTENSION}`);
	return { ...entry, path, text: utf8.decode(readFileSync(path)) };
}
