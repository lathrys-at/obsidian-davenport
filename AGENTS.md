...
- CI runs the lint, the typecheck, the tests with a coverage ratchet, the build with a bundle scan, a bundle-size check and a repeat-build check, and an advisory scan of the lockfile. CI does this on every commit on all branches. CI aggregates these jobs into the required `ci-ok` check.
...
- A separate workflow runs the mutation lane once a week, and on request. The lane is not part of `ci-ok`, and no merge waits for it. `npm run mutation` runs StrykerJS over the files under `src/`, and the run writes `reports/mutation/mutation.json`. `scripts/mutation-ratchet.mjs` reads that report. The script compares the score of the run against the floor that the committed `mutation-baseline.json` holds. The baseline holds one number, and that number is the score of the whole run. A score less than the floor fails the script, even by one hundredth of a point. The script also fails on a run that measured too little to score: a report that holds a mutant with a compile error or a runtime error, and a report whose mutants the score counts none of. Accept an intended change in the pull request that causes it: run `node scripts/mutation-ratchet.mjs --write-baseline` and commit the new file.
- The build writes `bundle-meta.json`, and `scripts/bundle-size.mjs` reads it. The script reports the raw size, the compressed size, and the modules that hold the most bytes. The script compares these numbers against the committed `bundle-baseline.json`. Two things fail the script. The first is growth past the baseline by more than a generous step. The second is an output file that the baseline holds and the build no longer makes. Accept an intended change in the pull request that causes it: run `node scripts/bundle-size.mjs --write-baseline` and commit the new file.
- `scripts/repeat-build.mjs` runs the build two times and compares what the two runs wrote. Before each run the script removes the metafile and every file that the metafile names, so each run starts with the output files of the build absent. Three things fail the script. The first is a file whose octets are not the same in the two runs. The second is a file that only one of the two runs wrote. The third is a build that declares no output file. A failure names the file, gives the place of the first octet that differs, and prints the octets of each run around that place. The script holds no baseline, and it needs no edit when the bundle changes. Run it with `node scripts/repeat-build.mjs`. The script leaves the files of its second run on disk, so the working tree ends as `npm run build` leaves it. The claim covers two runs on one machine with the pinned versions of Node and esbuild, and it covers nothing else.
