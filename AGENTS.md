...
- CI runs the lint, the typecheck, the tests with a coverage ratchet, and the build with a bundle scan and a bundle-size check. CI does this on every commit on all branches. CI aggregates these jobs into the required `ci-ok` check.
- The coverage run writes `coverage/coverage-summary.json`, and `scripts/coverage-ratchet.mjs` reads it. The script compares each source file against the floor that the committed `coverage-baseline.json` holds for that file. Two things fail the script. The first is one metric of one file that falls more than two percentage points below its floor. The second is a file that the baseline holds and the run no longer reports. Accept an intended change in the pull request that causes it: run `node scripts/coverage-ratchet.mjs --write-baseline` and commit the new file.
