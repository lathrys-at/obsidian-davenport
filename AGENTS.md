...
- CI runs the lint, the typecheck, the tests with a coverage ratchet, the build with a bundle scan and a bundle-size check, and an advisory scan of the lockfile. CI does this on every commit on all branches. CI aggregates these jobs into the required `ci-ok` check.
